...
public class SimApp {
	...
	public static void event(City mycity) {
		...
		// イベントリスト
		Event Remains = new Event("市内で歴史的な遺跡が発見！学者や観光客で賑わう", 20000, 0);
		events.add(Remains);
		Event Idols = new Event("ご当地アイドル「" + mycity.name + "娘」が全国的に大人気！", 10000, 0);
		events.add(Idols);
		...
		events.add(Ranking);
		Event Koshien = new Event("地元の高校が甲子園に出場！名門校として全国から生徒が集まる", 0, 10000);
		events.add(Koshien);
		Event Abroad = new Event("外国の姉妹都市から留学生を受け入れる", -10000, 10000);
		events.add(Abroad);
		Event Dam = new Event("集落を取り壊しダムを建設", 10000, -5000);
		events.add(Dam);
		Event Olympic = new Event("20XX年のオリンピック開催地に選ばれる！！！", 200000, 200000);
		events.add(Olympic);
		Event Tonde1 = new Event("ダサい街1位に選ばれる……", 0, -10000);
		events.add(Tonde1);
		Event Tonde2 = new Event("ダサい街1位に選ばれるも、それを逆手にとった映画\n「飛んで " + mycity.name + "」が大ヒット！", 30000, 10000);
		events.add(Tonde2);
		Event SchoolPool = new Event("学校職員がプールの水を止め忘れて大損失……", -10000, 0);
		events.add(SchoolPool);
		Event Typhoon = new Event("大型の台風が街を直撃し大きな被害を受ける……", -10000, -10000);
		events.add(Typhoon);
		Event Marriage = new Event("地元の神社に縁結びのご利益があると話題に！",10000,10000);
		events.add(Marriage);
		Event Virus1 = new Event("新型の感染症が流行し、対応に追われる",-30000,-30000);
		events.add(Virus1);
		Event Virus2 = new Event("新型の感染症が流行し、対応に追われるも\n通称「"+mycity.playerName+"ノマスク」を配布し、感染拡大を最小限に食い止める",-30000,0);
		events.add(Virus2);
		Event Squid1 = new Event("駅前にイカのオブジェを設置するも市民から絶不評……",-10000,-5000);
		events.add(Squid1);
		Event Squid2 = new Event("駅前に以下のオブジェを設置するも市民から不評……\nしかしSNSでは大バズリし、観光スポットに！",10000,-5000);
		events.add(Squid2);
